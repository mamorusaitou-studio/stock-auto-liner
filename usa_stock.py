...
def get_market_summary():
    ...
    for ticker, (name, desc) in INDICES.items():
        try:
            ...
            # 2. 【エラー回避の最終回答】
            # 表（DataFrame）から 'Close' 列の「値」だけを抜き出し、
            # .flatten() で銘柄名などのラベル情報を完全に消し去る
            # これで pure な数値だけのリストになる
            prices_list = df['Close'].values.flatten().tolist()
            
            # 3. リストからゴミ（NaN）を排除して純粋な数字の列を作る
            clean_prices = [float(x) for x in prices_list if str(x) != 'nan']
            
            if len(clean_prices) < 2:
                perf_text += f"\n◆ {name}\n   データ不足\n"
                continue

            close_now = clean_prices[-1]
            close_prev = clean_prices[-2]
            
            # 4. 年初来の取得 (インデックスの年が今年のものだけを抽出)
            ytd_df = df[df.index.year >= current_year]
            if not ytd_df.empty:
                ytd_list = ytd_df['Close'].values.flatten().tolist()
                clean_ytd = [float(x) for x in ytd_list if str(x) != 'nan']
                close_ytd = clean_ytd
            else:
                close_ytd = close_now
