...
def get_finance_data(ticker):
    try:
        ...
        data = res.json()
        result_main = data['chart']['result']
        timestamps = result_main.get('timestamp', [])
        
        # 修正の核心：quoteをリストとして正しく処理
        indicators = result_main.get('indicators', {})
        quote_list = indicators.get('quote', [])
        if not quote_list or not isinstance(quote_list, dict): return None
...
def get_market_summary():
    ...
    return text if success_found else None
